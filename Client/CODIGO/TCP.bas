...
'                End If
       ...
       Case "||"                 ' >>>>> Dialogo de Usuarios y NPCs :: ||
            Rdata = Right$(Rdata, Len(Rdata) - 2)
            Dim iuser As Integer, TmpColor As D3DCOLORVALUE
            iuser = Val(ReadField(3, Rdata, 176))

            If iuser > 0 Then
                Dialogos.CreateDialog ReadField(2, Rdata, 176), iuser, TmpColor
            Else
    ...
    Select Case Left$(sData, 3)
        ...
        Case "AGA" ' Aura Giratoria
            Rdata = Right$(Rdata, Len(Rdata) - 3)
            CharIndex = Val(ReadField(1, Rdata, 44))
            charlist(CharIndex).Aura_Index = Val(ReadField(2, Rdata, 44))
            Call InitGrh(charlist(CharIndex).Aura, Val(ReadField(2, Rdata, 44)))
            charlist(CharIndex).AuraAngle = 1
        Case "HMS"                  ' >>>>> Horas, Minutos, Segundos :: HMS ShaFTeR
            Rdata = Right$(Rdata, Len(Rdata) - 3)
            HorasClima = Val(ReadField(1, Rdata, 44))
            MinutosClima = Val(ReadField(2, Rdata, 44))
            SegundosClima = Val(ReadField(3, Rdata, 44))
            Exit Sub
        Case "BKW"                  ' >>>>> Pausa :: BKW
            pausa = Not pausa
            Exit Sub
